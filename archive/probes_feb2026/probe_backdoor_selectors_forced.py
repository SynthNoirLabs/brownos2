#!/usr/bin/env python3
"""
Backdoor selector brute-force (force the dummy arg).

Backdoor returns a Scott cons cell:
  pair = λc.λn. c A B

So (pair s) is a *function* (λn. s A B). If the intended "3-leaf" record is
about the actual application `s A B`, we must force the unused `n` too:
  (pair s) NIL  ==>  s A B

This script runs the same small selector set as probe_backdoor_selectors.py,
but calls syscall 8 with the *forced* derived term.
"""
...
def build_program(selector_name: str, selector: object) -> object:
    ...
    Program:
      backdoor(nil) >>= \\either.
        either
          (\\pair. write(label) >>= \\_. 8(((pair selector) NIL)) CONT_AFTER8)
          (\\_.   write(\"BDR\\n\") >>= \\_. I)
    """
    ...
    derived_raw = App(Var(1), selector_s3)  # (pair selector) => λn. selector A B
    derived = App(derived_raw, nil_s3)  # force dummy n => selector A B

    call8 = App(App(Var(0x08 + 3), derived), cont_after8_s3)
...
def main() -> None:
    ...
    print(f"Running {len(selectors)} forced selectors...")
    print("Output format per case: label, then 'R', then error text (or hang/empty).\n")
