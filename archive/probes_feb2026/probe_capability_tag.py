...
def from_backdoor(pair_use_builder) -> object:
    return apps(
        g(201),
        NIL,
        lam(
            ...
            apps(
                v("bd_res"),
                lam("pair", pair_use_builder(v("pair"))),
...
def list2_named(x: object, y: object) -> object:
    return NConst(make_scott_list([to_db(x), to_db(y)]))
...
def main() -> None:
    print("=" * 80)
    print("probe_capability_tag.py")
    print("Capability-tag tests with runtime backdoor values (pair/A/B)")
    print("=" * 80)
    print()
    ...
    # Controls
    run_case("CTRL1: g(8)(nil)(OBS)", apps(g(8), NIL, OBS), rows)
    run_case("CTRL2: g(42)(nil)(OBS)", apps(g(42), NIL, OBS), rows)

    # A-group: runtime pair/A/B as argument to real sys8 (tag-like capability proxy)
    run_case(
        "A1: backdoor -> sys8(pair)(OBS)",
        from_backdoor(lambda pair: apps(g(8), pair, OBS)),
        rows,
    )
    run_case(
        "A2: backdoor -> sys8(A)(OBS)",
        from_backdoor(lambda pair: apps(g(8), apps(pair, selector_a(), NIL), OBS)),
        rows,
    )
    run_case(
        "A3: backdoor -> sys8(B)(OBS)",
        from_backdoor(lambda pair: apps(g(8), apps(pair, selector_b(), NIL), OBS)),
        rows,
    )

    # B-group: pair controls both argument and continuation to sys8
    run_case(
        "B1: backdoor -> pair(g8)(nil)",
        from_backdoor(lambda pair: apps(pair, g(8), NIL)),
        rows,
    )
    run_case(
        "B2: backdoor -> pair(\\x.g8)(nil)",
        from_backdoor(lambda pair: apps(pair, lam("x", g(8)), NIL)),
        rows,
    )

    # C-group: compound password + capability
    pw = NConst(encode_bytes_list(b"ilikephp"))
    run_case(
        "C1: backdoor -> sys8(ilikephp)(OBS)",
        from_backdoor(lambda _pair: apps(g(8), pw, OBS)),
        rows,
    )
    run_case(
        "C2: backdoor -> sys8(pair(ilikephp))(OBS)",
        from_backdoor(lambda pair: apps(g(8), apps(pair, pw), OBS)),
        rows,
    )
    ...
    if breakthroughs:
        print("\nBREAKTHROUGH:")
        for label, bucket, _elapsed, hx in breakthroughs:
